    ...
    def _copy_draft_token_ids_to_cpu(
        ...
        with torch.cuda.stream(self.draft_token_ids_copy_stream):
            if not zeros_only:
                ...
                # Copy actual draft tokens to fixed-size buffer
                self.draft_token_ids_cpu[:num_reqs, :num_draft_tokens].copy_(
                    draft_token_ids, non_blocking=True
                )
                # Pad the rest with zeros if draft tokens are shorter than buffer
                if num_draft_tokens < self.num_spec_tokens:
                    self.draft_token_ids_cpu[:num_reqs, num_draft_tokens:].fill_(0)
            else:
